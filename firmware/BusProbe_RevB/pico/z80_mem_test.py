...
import bus
from z80_async_bus import do_cmd  # Import our new async wrapper
...
def run(pic, bus_mgr, output):
    ...
    # Ensure safe starting state using the new async wrapper with error raising enabled
    bus_mgr.ghost_all()
    ...
    for window_start_page in range(0, TOTAL_PHYSICAL_PAGES, PAGES_PER_WINDOW):
        ...
        for logical_page in range(PAGES_PER_WINDOW):
            ...
            output(
                f"  ... Written Page {physical_page:03d}/255 [Write Speed: {bytes_per_sec:.0f} B/s | Global ETA: {format_eta(eta_seconds)}]")
        ...
        for window_start_page in range(0, TOTAL_PHYSICAL_PAGES, PAGES_PER_WINDOW):
            ...
            for logical_page in range(PAGES_PER_WINDOW):
                ...
                for offset in range(BYTES_PER_PAGE):
                    ...
                    # Async wrapper silently handles the QUEUED -> STATUS polling loop
                    resp = do_cmd(pic, ["READ", f"{logical_addr:04X}"])

                    if resp.startswith("OK"):
                        try:
                            # Because do_cmd normalizes "OK DONE 42" to "OK 42", this parse still works perfectly!
                            read_val = int(resp.split()[1], 16)
                            if read_val != expected_val:
                                output(
                                    f"MISMATCH at Physical 0x{physical_addr:05X} (Logical 0x{logical_addr:04X}): Expected 0x{expected_val:02X}, Read 0x{read_val:02X}")
                                errors += 1
                        except IndexError:
                ...
                output(
                    f"  ... Verified Page {physical_page:03d}/255 [Read Speed: {bytes_per_sec:.0f} B/s | Global ETA: {format_eta(eta_seconds)}]")
