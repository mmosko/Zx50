...
module z80_cpu_util (
    ...
    initial begin
        ...
        data_out = 8'h00;
        drive_data = 0;
        mreq_n = 1; iorq_n = 1; rd_n = 1; wr_n = 1; m1_n = 1; reset_n = 1;
    end

    // ==========================================
    // TASK: Reset Sequence
    // Reset Cycle. RESET must be active for at least three clock cycles.   
    // While RESET remains active, the address and data buses float, and the
    // control outputs are inactive. Once RESET goes inactive, two internal T
    // cycles are consumed before the CPU resumes normal processing operation.
    // RESET clears the PC so the first opcode fetch will be to location 0x0000.
    // ==========================================
    task boot_sequence();
        begin
            // Boot Sequence 
            reset_n = 1;      
            wait_cycles(5); // Wait a few cycles to ensure stable idle state
            reset_n = 0;
            // TODO: float control outputs after brief delay 
            mreq_n = 1'bz;
            iorq_n = 1'bz;
            rd_n = 1'bz;
            wr_n = 1'bz;
            ...
            wait_cycles(5); // Hold reset for a few cycles
            reset_n = 1; 
