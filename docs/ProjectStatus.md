...
1. **Zx50 Bus Probe Hardware (In Progress)**:
   * **Assembly**: Receiver/Sender boards are partially assembled. 74LVC138A (U7) is perfectly flowed. 74LVC154A (U14) is placed but requires minor bridge cleanup on the lower pins.
   * **Tooling**: PyCharm MicroPython environment is fully configured with the correct "Basic Pico" Run/Upload profile to persist code across power cycles.

2. **Zx50 Bus Probe Pico Firmware (Completed)**:
   * **Modular Architecture**: Re-architected the Pico firmware into strictly encapsulated modules (`bus.py`, `pic18_link.py`, `pins.py`) utilizing the Command/Dispatcher pattern.
   * **RPC Link (`pic18_link.py`)**: Implemented the 1 Mbps UART bridge to the PIC18F4620, supporting all 5-byte binary RPC commands (READ, WRITE, IN, OUT, STEP) as well as the dynamic payload commands (LDIR, SNAPSHOT).
   * **Interactive REPL (`main.py`)**: Built a lightweight, non-blocking `cmd.Cmd`-style terminal parser using Python reflection (`getattr`). Features subsystem routing, nested help menus (`pic help`, `bus help`), and a professional boot banner reporting the MicroPython build version.
   * **High-Speed GPIO (`pins.py`)**: Implemented direct memory access (`machine.mem32[0xd0000004]`) for single-cycle, 8-bit parallel bus reads.
...
## 🚧 Immediate Blocker
* **SMT Rework**: Need to clear the solder bridges on the 74LVC154A (U14) using flux-soaked wick before the board can be safely powered on for multiplexer testing. 

## 🚀 Next Steps (To-Do List)

### 1. Bus Probe Hardware Bring-Up
* **Clear Bridges:** Use Chip Quik CQ4LF (liquid flux) heavily applied to copper wick to clear the lower pins on the 154 multiplexer. 
* **Flash & Test Bus Mux:** Flash the completed `main.py` ecosystem to the Pico, connect via serial, and test the `bus select` commands to verify the LEDs sequence correctly across D1-D32.
* **Test RPC Link:** Verify the 1 Mbps UART connection between the Pico and the PIC18F4620 using the `pic read` and `pic write` terminal commands.
