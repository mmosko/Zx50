...
Revision C2 updates the design with an upgraded **Math and Stack Coprocessor Cluster** mounted on the bottom side beneath the clock mezzanine shield, expanding private coprocessor memory to **32 KB** and introducing a **12 ns high-speed SRAM** to guarantee single-cycle memory operations at 40 MHz.
...
```text
         ...
         Private Address|                       |  Private Data
            ...
            |   | 32KB Active (55ns)|   | 32KB Active   |   |
            |   | PLCC-32 (U13)     |   | SRAM 28-TSOP1 |   |
            |   +---------+---------+   +-------+-------+   |
            |             |                     |           |
            |             |     ~C_OE / ~C_WE   |           |
            |             +----------+----------+           |
...
The CPLD manages its own dedicated private address (`CA0`–`CA14`) and data (`CD0`–`CD7`) buses, isolating coprocessor memory operations from the primary Z80 system bus:

* **Expanded Private Address Space (32 KB):** A full 15-bit address bus (`CA0`–`CA14`) provides **32 KB** of addressable private memory. Upper unused address lines (`A15`–`A18`) on the Flash ROM (`U13`) are tied directly to **`GND`** to guarantee static CMOS levels.
* **Private Flash/EEPROM (`SST39SF040` / `U13`):** 32 KB active window (55 ns NOR Flash) in a 32-pin PLCC footprint (`CONN_PLCC-32-AT-SMT_ADM`). Stores non-volatile lookup tables (Quarter-Square $f(n)=\lfloor n^2/4 \rfloor$, sine/cosine maps, logarithmic tables, reciprocal seeds).
* **Private High-Speed SRAM (`IS61C256AL-12TLI` / `U12`):** 32 KB active window (12 ns High-Speed Static RAM) in a 28-pin TSOP-I footprint. Operates as a private scratchpad, hardware stack frame storage, or modifiable shadow RAM.
* **Consolidated Control Busses:** Read enable (`~C_OE`, Pin 45) and write strobe (`~C_WE`, Pin 46) are shared across both memory ICs. Access targeting is handled via dedicated Chip Enable lines (`~M_CE` on Pin 44 for SRAM; `~F_CE` on Pin 68 for Flash).
...
* **Jumper Open (High / Default):** Standard mode.
* **Jumper Shorted to GND (Low):** High-speed 1-cycle pipeline selection.
...
Revision C2 uses a **6-layer stackup** to accommodate the high-density coprocessor cluster beneath the top-side clock shield without interfering with front-side copper:
...
| **Layer 3 (Inner 1)** | Signal | **Private Coprocessor Bus:** `CA0`–`CA14`, `CD0`–`CD7`, and private control lines (`~M_CE`, `~F_CE`, `~C_OE`, `~C_WE`) |
...
| **Layer 6 (Bottom)** | Signal / SMT | Bottom-mounted SMT footprints: ATF1508AS PLCC-84 (`U11`), SST39SF040 PLCC-32 (`U13`), IS61C256AL TSOP-I-28 (`U12`) |
...
| **`J8` - MEM SPD Jumper** | 1x2 Header (2.54mm) | Hardware jumper for private memory access speed selection (`Pin 81` / `R5`). |

---

## 8. Estimated Power Consumption Analysis

Operating at $5\text{V}$, the Rev C2 CPU card draws a typical active current of **~170 mA (~0.85 W)** and a worst-case peak switching current of **~280 mA (~1.40 W)**. The power load is split between the primary Z80 CPU subsystem and the high-speed coprocessor cluster.
...
| **Private SRAM (12 ns)** | `IS61C256AL-12TLI` | 1 | 15 mA | 30 mA | Active read/write state |
