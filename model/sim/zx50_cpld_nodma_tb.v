`timescale 1ns/1ps

/***************************************************************************************
 * MODULE: zx50_cpld_nodma_tb
 * DESCRIPTION:
 * [REVISION: "No-DMA" Fallback Variant. Tests the zx50_cpld_nodma core to ensure
 * basic Z80 MMU and memory transceiver logic functions flawlessly without the DMA
 * or Arbiter modules present.]
 ***************************************************************************************/

module zx50_cpld_nodma_tb;

    // --- 1. System Clocks (Generated by the Timebase) ---
    wire mclk, zclk;
    zx50_clock clk_gen (
        .run_in(1'b1),
        .step_n_in(1'b1),
        .mclk(mclk),
        .zclk(zclk)
    );

    // --- 2. System Signals ---
    reg reset_n;
    reg z80_iei;
    reg [3:0] card_id_sw;

    // --- 3. Z80 Backplane Buses (Driven by BFM) ---
    wire [15:0] z80_addr;
    wire [7:0]  z80_data;
    wire z80_mreq_n, z80_iorq_n, z80_rd_n, z80_wr_n, z80_m1_n;
    wire z80_wait_n, z80_ieo, z80_int_n;

    // [REVISION: Section 4 (Shadow DMA Bus) completely removed from this testbench.]

    // --- 5. CPLD Output Pins to Monitor ---
    wire [10:0] l_addr;
    wire [3:0]  atl_addr; // [REVISION: Shrunk to 4 bits]
    wire [7:0]  atl_data;
    wire atl_we_n, atl_oe_n, atl_ce_n;
    wire ram_ce0_n, ram_ce1_n, ram_oe_n, ram_we_n;
    wire z80_data_oe_n, l_dir;
    
    // [REVISION: sh_data_oe_n and sh_c_dir removed from monitor list]

    // --- 6. The Duplex Handoff Logic ---
    wire [3:0] duplex_bus = (reset_n) ? {z80_iorq_n, z80_mreq_n, z80_wr_n, z80_rd_n} : card_id_sw;

    // --- 7. BFM Instantiations ---

    // Z80 Master
    z80_cpu_util z80 (
        .clk(zclk), .addr(z80_addr), .data(z80_data),
        .mreq_n(z80_mreq_n), .iorq_n(z80_iorq_n), 
        .rd_n(z80_rd_n), .wr_n(z80_wr_n), .m1_n(z80_m1_n),
        .wait_n(z80_wait_n) 
    );

    // [REVISION: The Device Under Test is now the NoDMA CPLD core]
    zx50_cpld_nodma dut (
        .mclk(mclk), .reset_n(reset_n), 
        .duplex_in(duplex_bus), 
        .z80_m1_n(z80_m1_n), .z80_iei(z80_iei), .z80_ieo(z80_ieo),
        .z80_int_n(z80_int_n), .z80_wait_n(z80_wait_n),
        
        .z80_addr(z80_addr), .l_data(z80_data), 
        
        // [REVISION: Shadow bus mappings removed from instantiation]
        
        .l_addr(l_addr), .atl_addr(atl_addr), .atl_data(atl_data),
        .atl_we_n(atl_we_n), .atl_oe_n(atl_oe_n), .atl_ce_n(atl_ce_n),
        .ram_ce0_n(ram_ce0_n), .ram_ce1_n(ram_ce1_n), .ram_oe_n(ram_oe_n), .ram_we_n(ram_we_n),
        .z80_data_oe_n(z80_data_oe_n), .l_dir(l_dir)
    );

    // We MUST include the LUT here. If atl_data is floating, the CPLD 
    // won't know how to resolve atl_data[7], turning ram_ce0_n/ram_ce1_n into 'x'.
    // During an MMU write, route the local data to the LUT model.
    assign atl_data = (!atl_we_n) ? z80_data : 8'hzz;
    
    // [REVISION: LUT Address padded with 11 zeros to fit 4-bit atl_addr into 15-bit SRAM model]
    is61c256al lut_sram (
        .addr({11'b0, atl_addr}), .data(atl_data),
        .ce_n(atl_ce_n), .oe_n(atl_oe_n), .we_n(atl_we_n)
    );

    // --- 8. Test Sequence ---
    reg [7:0] dummy_data;

    initial begin
        // [REVISION: Updated dumpfile name to isolate NoDMA wave results]
        $dumpfile("waves/zx50_cpld_nodma.vcd");
        $dumpvars(0, zx50_cpld_nodma_tb);

        // Setup
        z80_iei = 1;
        card_id_sw = 4'hC; // Let's use Card ID 0xC (12)
        
        // Boot Sequence: Mimic an RC reset circuit
        reset_n = 1;  // Power on
        #50 reset_n = 0;  // RC circuit holds reset low
        #700 reset_n = 1; // RC circuit charges, releasing reset

        // Wait for the 16-cycle Hardware Wipe to finish
        clk_gen.wait_mclk(20);

        // ==========================================
        // PHASE 1: Program MMU
        // ==========================================
        $display("[%0t] Phase 1: Programming MMU Page 0 to 0x45...", $time);
        z80.io_write(16'h003C, 8'h45); 

        z80.wait_cycles(5); 

        // ==========================================
        // PHASE 2: CPLD Output Validation (Write Cycle)
        // ==========================================
        $display("[%0t] Phase 2: Memory Write Validation...", $time);
        
        fork
            z80.mem_write(16'h0123, 8'hAA);
            begin
                wait(z80_wr_n == 1'b0); 
                #10;

                // Verify transceivers are open and pointing inward
                if (z80_data_oe_n !== 1'b0 || l_dir !== 1'b1) begin
                    $display("FATAL: Transceiver write logic failed! oe_n=%b, l_dir=%b", z80_data_oe_n, l_dir);
                    $fatal(1);
                end
                
                // Verify the correct Chip Enable fired
                if (ram_ce0_n !== 1'b0 || ram_ce1_n !== 1'b1 || ram_we_n !== 1'b0) begin
                    $display("FATAL: Main SRAM gating failed! ce0=%b, ce1=%b, we=%b", ram_ce0_n, ram_ce1_n, ram_we_n);
                    $fatal(1);
                end
                
                $display("--- Phase 2 Passed: CPLD routed write signals perfectly.");
            end
        join

        z80.wait_cycles(5);

        // ==========================================
        // PHASE 3: CPLD Output Validation (Read Cycle)
        // ==========================================
        $display("[%0t] Phase 3: Memory Read Validation...", $time);
        
        fork
            z80.mem_read(16'h0123, dummy_data);
            begin
                wait(z80_rd_n == 1'b0); 
                #10;

                // Verify transceivers are open and pointing outward
                if (z80_data_oe_n !== 1'b0 || l_dir !== 1'b0) begin
                    $display("FATAL: Transceiver read logic failed! oe_n=%b, l_dir=%b", z80_data_oe_n, l_dir);
                    $fatal(1);
                end
                
                // Verify Output Enable fired
                if (ram_ce0_n !== 1'b0 || ram_oe_n !== 1'b0) begin
                    $display("FATAL: Main SRAM read gating failed! ce0=%b, oe=%b", ram_ce0_n, ram_oe_n);
                    $fatal(1);
                end
                
                $display("--- Phase 3 Passed: CPLD routed read signals perfectly.");
            end
        join

        z80.wait_cycles(5);
        $display("=====================================================");
        $display(" SUCCESS: NoDMA CPLD Core Logic is fully operational.");
        $display("=====================================================");
        $finish;
    end

    // --- System Watchdog Timer ---
    initial begin
        #150000;
        $display("FATAL [%0t]: Watchdog Timer Expired!", $time);
        $fatal(1);
    end
endmodule