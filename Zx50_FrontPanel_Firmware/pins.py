...
SPI_BUS = SPI(0, sck=Pin(16), mosi=Pin(17))
...
def setup_spi_for_hcms_and_leds():
    ...
    SPI_BUS.init(baudrate=5_000_000, polarity=0, phase=0)

# SPI Device Control Pins
LCD_CS  = Pin(18, Pin.OUT, value=1)      # Chip Select for character LCD (Active Low)
HCMS_CE = Pin(19, Pin.OUT, value=1)      # Chip Enable for HCMS displays (Active Low)
